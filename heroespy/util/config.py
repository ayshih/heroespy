...
import heroespy as hpy

__all__ = ['load_config', 'print_config']

def load_config():
    """
    Read the heroespyrc configuration file. If one does not exists in the user's
    home directory then read in the defaults from module
    """
    config = ConfigParser.SafeConfigParser()

    # Get locations of HEROESpy configuration files to be loaded
    ...
    if not config.has_option('general', 'working_dir'):
        config.set('general', 'working_dir', os.path.join(_get_home(), "heroespy"))
...
def _find_config_files():
    """Finds locations of HEROESpy configuration files"""
    config_files = []
...
def _get_user_configdir():
    ...
    The default is "HOME/.heroespy".  You can override this with the
    HEROESPY_CONFIGDIR environment variable
    """
    configdir = os.environ.get('HEROESPY_CONFIGDIR')

    if configdir is not None:
        if not _is_writable_dir(configdir):
            raise RuntimeError('Could not write to HEROESPY_CONFIGDIR="%s"' %
                               configdir)
        return configdir

    h = _get_home()
    p = os.path.join(_get_home(), '.heroespy')

    if os.path.exists(p):
        if not _is_writable_dir(p):
            raise RuntimeError("'%s' is not a writable dir; you must set %s/."
                               "heroespy to be a writable dir.  You can also set "
                               "environment variable HEROESPY_CONFIGDIR to any "
                               "writable directory where you want matplotlib "
                               "data stored " % (h, h))
    else:
        if not _is_writable_dir(h):
            raise RuntimeError("Failed to create %s/.heroespy; consider setting "
                               "HEROESPY_CONFIGDIR to a writable directory for "
                               "heroespy configuration data" % h)
